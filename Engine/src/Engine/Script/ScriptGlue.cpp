...
#include <VolcaniCore/Window/KeyEvents.h>
#include <VolcaniCore/Window/MouseEvents.h>
...
static std::string AssetGetName(Asset* asset) {
	return AssetManager::Get()->GetRegistry()->GetAssetName(*asset);
}
...
void RegisterAssetManager() {
	...
	engine->RegisterEnumValue("AssetType", "Material", 5);
